...
> 研究日期：2026-07-14
> 消息编排证据固定到 `5dc9490bb35f9729ef2c95d00a19ccd30c26339c`；较早的架构综述内容保留作背景。
...
## 17. 消息编排（固定 commit `5dc9490`）

本节补充源码固定证据：Aider 标准 coding loop 使用**prompt 约定的结构化编辑文本**，而不是 provider-native tools。所有链接使用 [`Aider-AI/aider@5dc9490bb35f9729ef2c95d00a19ccd30c26339c`](https://github.com/Aider-AI/aider/commit/5dc9490bb35f9729ef2c95d00a19ccd30c26339c)。另见横向文档 [agent-message-orchestration.cn.md](agent-message-orchestration.cn.md)。

### 17.1 Prompt、history 与无工具路径

`cur_messages` 保存当前用户 task 及反思重试，`done_messages` 保存结束任务。每次按 `system → examples → readonly → repo map → done → current chat files → cur → reminder` 重新构造并全量调用 LiteLLM。见 [`chat_chunks.py#L16-L55`](https://github.com/Aider-AI/aider/blob/5dc9490bb35f9729ef2c95d00a19ccd30c26339c/aider/coders/chat_chunks.py#L16-L55)、[`base_coder.py#L1419-L1466`](https://github.com/Aider-AI/aider/blob/5dc9490bb35f9729ef2c95d00a19ccd30c26339c/aider/coders/base_coder.py#L1419-L1466)。

system prompt 按 coder 编辑格式动态构造；例如 EditBlockCoder 要求 assistant 输出 SEARCH/REPLACE 文本，本地 parser 解析并应用。见 [`editblock_prompts.py#L7-L30`](https://github.com/Aider-AI/aider/blob/5dc9490bb35f9729ef2c95d00a19ccd30c26339c/aider/coders/editblock_prompts.py#L7-L30)、[`editblock_coder.py#L15-L41`](https://github.com/Aider-AI/aider/blob/5dc9490bb35f9729ef2c95d00a19ccd30c26339c/aider/coders/editblock_coder.py#L15-L41)。

标准 `Coder.functions=None`；只有非标准/遗留 coder 设置 functions 时才发送 tools，当前 registry 中 func coder 已被注释。见 [`base_coder.py#L88-L102`](https://github.com/Aider-AI/aider/blob/5dc9490bb35f9729ef2c95d00a19ccd30c26339c/aider/coders/base_coder.py#L88-L102)、[`coders/__init__.py#L16-L34`](https://github.com/Aider-AI/aider/blob/5dc9490bb35f9729ef2c95d00a19ccd30c26339c/aider/coders/__init__.py#L16-L34)。

### 17.2 Provider 差异

provider 层统一走 LiteLLM，但 prompt 并非完全同形：部分 OpenAI reasoning 模型禁用 system role，Aider 将系统内容放入 user 并补 assistant `"Ok."`；Anthropic 模型可在若干 prompt 边界设置 ephemeral cache control，某些 Sonnet reminder 可追加到最后 user 消息。见 [`models.py#L470-L491`](https://github.com/Aider-AI/aider/blob/5dc9490bb35f9729ef2c95d00a19ccd30c26339c/aider/models.py#L470-L491)、[`chat_chunks.py#L28-L55`](https://github.com/Aider-AI/aider/blob/5dc9490bb35f9729ef2c95d00a19ccd30c26339c/aider/coders/chat_chunks.py#L28-L55)。模型普通输出取 assistant `content`，若 adapter 提供 reasoning 字段则可单独展示，但编辑 parser 仍以 content 为准。

### 17.3 反思、摘要与 architect 组合

解析/应用错误会成为新的 synthetic user 反思消息，加入 `cur_messages` 后再次全量请求；不是 tool result。结束时当前 messages 并入 done。历史摘要器只压缩较旧 done head，保留近期 tail；repo map 有独立 token 预算。见 [`history.py#L33-L123`](https://github.com/Aider-AI/aider/blob/5dc9490bb35f9729ef2c95d00a19ccd30c26339c/aider/history.py#L33-L123)。

Architect mode 建立新的 editor Coder 并把 architect 方案作为首条 user 消息，是顺序 planner→editor，不是通用 subagent。见 [`architect_coder.py#L11-L48`](https://github.com/Aider-AI/aider/blob/5dc9490bb35f9729ef2c95d00a19ccd30c26339c/aider/coders/architect_coder.py#L11-L48)。
...
If parse failed, Request 2:
  full rebuilt context
  + user U1
  + assistant A1
...
### 17.5 并行策略、证据限制与当前 Marix 借鉴

Aider 标准路径不发送 native tool batch，因此不适用并行 native-tool 执行策略；host 每次解析并应用一份 assistant 编辑响应。这不能证明可选命令或非标准 coder 的通用并发策略。

上述结论只适用于固定 SHA 下的 coder registry 与 prompt assembly。部分非标准或遗留 coder 可以设置 `functions`，因此不能把“无 native tools”推广到所有 Aider 扩展路径。

Marix 可把 prompt-defined editing 保留为 native tools 之外的有意选择，并将其建模为 assistant output 后接 host application/audit event，而不是伪造 provider tool call 或 tool-result message。

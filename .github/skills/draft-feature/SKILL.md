...
This skill is for API shaping and review only. It may add or edit Rust source files, modules, traits, structs, enums, type aliases, constants, and public function or method signatures, but it must not add working behavior.
...
4. **Use Non-Implementation Stubs** — Where Rust requires a function body, the body must be only:

   ```rust
   panic!("not implemented")
   ```

   Trait methods should use signature-only declarations with semicolons unless a body is strictly required by the API shape.
...
- Do not add tests for behavior. If tests are necessary for compilation shape, ask the user before adding them.
...
- Function and method signatures are compilable with `panic!("not implemented")` bodies where bodies are required.
...
- whether any required function bodies are `panic!("not implemented")` stubs.

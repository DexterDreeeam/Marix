...
4. **Refresh Overview** — If any overview UI, documentation, module structure, source layout, or diff behavior changed, use the `overview-engineer` to update all overview content. The overview engineer owns file view, star-map view, bilingual UI, module hierarchy, diff annotations, and module details.
5. **Stage & Commit** — If there are uncommitted changes after overview refreshes, run `git add -A` and commit with a concise English message (20 words or fewer). Always include the trailer: `Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>`.
...
- Overview refreshes and diff summaries must treat every dot-prefixed file or folder under `src/` as companion metadata maintained by `development-designer`, not as visible source content.

...
4. **Build Pages** — Run `powershell -ExecutionPolicy Bypass -File scripts/build_pages.ps1` to regenerate the GitHub Pages file explorer with the latest diff data. Stage and commit the updated `docs/manifest.json` with the message "Update pages manifest".

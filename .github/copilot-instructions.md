...
- Dot-prefixed files and folders under `src/` are companion metadata: `.design.json` is maintained by `development-designer`, and `.workflow.mmd` by the explicit `update-workflow` skill. Do not treat them as normal source files in overview file trees or {{proj\_lower}} tag diffs.

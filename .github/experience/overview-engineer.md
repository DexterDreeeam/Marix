...
- 2026-06-23: Active-tab reload banner UX was simplified in `app-active-reload-banner.js`/`app-config.js`: no Later/dismiss button, one Reload/重载 action that directly calls `window.location.reload()`, concise bilingual copy, 5-second auto-hide countdown, and the same 30-second successful-display cooldown. Keep it passive: no file scanning or change detection.

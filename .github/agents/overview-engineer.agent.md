...
The overview page must not depend on checked-in manifest files. Data source selection is URL-routed, not `localStorage`-routed: root paths show a two-option source picker with no local path text input, `/remote` loads GitHub, `/local` uses the browser-selected local repository handle, and `/local/...` decodes an explicit local path from URL segments. GitHub mode builds data dynamically from GitHub repository tree and `{{proj_lower}}_tag_*` compare APIs. Local mode uses the File System Access API and an IndexedDB-cached directory handle keyed by the route path; if the URL-selected local folder cannot be read, clear that handle and ask the user to grant access again. Keep the reset-data-source button immediately to the right of the language switch button.

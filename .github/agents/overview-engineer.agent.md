...
Publicly exposed items from source-design companion metadata `exposedGroups` should be scattered across the star map as smaller grouped nodes. Traits use deep-pink triangles. Struct definitions use blue squares. Functions use purple circles. Enums and other small definitions such as type aliases, constants, statics, and globals use less-prominent dark-blue five-point stars. Clicking an exposed element opens its code snippet in the code popover with language highlighting.

Exposed element fill colors are type colors, not status colors. Use blue, purple, pink, and white families to distinguish exposed element types. Do not use green, yellow, or red as exposed element fill colors because those colors are reserved for change status. Status may be shown through outlines, badges, borders, and ordering. Reopening a code popover must preserve syntax highlighting by resetting highlight state before re-highlighting.
...
Module property panels must render source-design companion metadata. Module panels should show only non-empty sections: child modules plus exposed element sections promoted to the top level by exact kind, such as Trait, Function, Struct, and Enum. Do not merge traits and functions into a generic "Public interfaces" section. Within every section, changed items must sort above unchanged items. Child module, interface, and type rows in the module panel should be compact and show names only; detailed signatures, paths, source paths, and prose details belong in the code popover or file popover, not in the panel. File path lists should not render internal file details in the right panel. Code snippets open by clicking the whole interface/type block, and clicking outside the popover hides it.

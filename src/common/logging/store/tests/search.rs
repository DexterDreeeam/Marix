...
fn unicode_trigrams_are_session_isolated_and_fully_verified() {
    ...
    let expected = store
        .record(&message(Some(session), LogTag::Info, 100, "ÜBER Straße"))
        .expect("record Unicode match");
    store
        .record(&message(Some(session), LogTag::Info, 90, "abc gap bcd"))
        .expect("record trigram false positive");
    store
        .record(&message(Some(other), LogTag::Info, 110, "über straße"))
...
fn absent_long_keyword_uses_no_session_time_scan() {
    ...
    let session = uuid::Uuid::new_v4();
    store
        .record(&message(Some(session), LogTag::Info, 10, "ordinary entry"))
        .expect("record entry");
...
#[test]
fn long_keyword_pages_verify_tag_and_stable_time_id_cursors() {
    let store = Store::open_at(&temp_path()).expect("open store");
    let session = uuid::Uuid::new_v4();
    store
        .record(&message(
        ...
        .expect("record trigram false positive");
    let older = store
        .record(&message(Some(session), LogTag::Info, 100, "needle older"))
        .expect("record older match");
    let tied_older = store
        .record(&message(Some(session), LogTag::Info, 300, "needle tied"))
        .expect("record tied older match");
    let tied_newer = store
        .record(&message(Some(session), LogTag::Info, 300, "NEEDLE newest"))
    ...
    store
        ...
        .expect("record other tag");

    let mut request = query(Some(session));
    request.tag = Some(LogTag::Info);
...
fn long_keyword_incremental_pages_advance_by_session_record_id() {
    ...
    let other = uuid::Uuid::new_v4();
    let base = store
        .record(&message(Some(session), LogTag::Info, 1, "base"))
        .expect("record base");
    store
        .record(&message(Some(other), LogTag::Info, 100, "needle other"))
        .expect("record other session");
    store
        .record(&message(
            Some(session),
        ...
        .expect("record other tag");
    let first = store
        .record(&message(Some(session), LogTag::Info, 10, "NEEDLE first"))
        .expect("record first match");
    store
        .record(&message(Some(session), LogTag::Info, 80, "not a match"))
        .expect("record sparse miss");
    let second = store
        .record(&message(Some(session), LogTag::Info, 30, "needle second"))
        .expect("record second match");
    let third = store
        .record(&message(Some(session), LogTag::Info, 20, "needle third"))
        .expect("record third match");
    let latest = store
        .record(&message(Some(other), LogTag::Info, 200, "latest other"))
...
fn short_keyword_uses_session_index_without_trigram_table() {
    ...
    let session = uuid::Uuid::new_v4();
    let expected = store
        .record(&message(Some(session), LogTag::Info, 10, "ÜB match"))

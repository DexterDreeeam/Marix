...
use marix_protocol::{
    PlanDraft, PlanSignature, StepDraft, StepSignature,
};
...
impl IntentRuntime {
    pub(super) fn create_step(&self, draft: StepDraft) -> Result<(), String> {
        ...
        let signature = StepSignature::new(
            ...
            format!("step-{}", self.state.steps.size() + 1),
        );
        ...
        self.state.steps.insert(signature, None);
        step.start();
        Ok(())
    }
    ...
    pub(super) fn create_plan(&self, draft: PlanDraft) -> Result<(), String> {
        ...
        let plan_signature =
            PlanSignature::new(self.state.signature.clone(), "plan".to_owned());
        ...
        for (index, draft) in draft.intents.into_iter().enumerate() {
            ...
            if !self.state.access.insert_intent(intent.clone()) {
        ...
        let plan = Plan::new(
            Arc::clone(&self.state.access),
            plan_signature.clone(),
            intents,
        );
        if !self.state.access.insert_plan(plan.clone()) {
